...
class Poliomino:
    ...
    def __init__(self, s1: int, s2: int):
        self.__pol = np.array([[i, j] for i in range(s2) for j in range(s1)])

    def rotate(self):
        self.__pol = np.dot(self.__pol, Poliomino.__T)

    def shift(self, a: int, b: int):
        return self.__pol + np.array([a, b])
...
class LPoliomino(Poliomino):
    def __init__(self, q1: int, q2: int):
        ...
        self.__pol = np.array([[0, i] for i in range(q1)] + [[j, 0] for j in range(1, q2)])
